...
__global__ void cudaNaiveDCT2DKernel(const uint numTotalThreads, const uint batchSizeDim,
      ...
      for (uint v = 0; v < numPoints; v++) {
        ...
        for (uint i = 0; i < numPoints; i++) {
          ...
          for (uint j = 0; j < numPoints; j++) {
            ...
            output[spectralIdx] +=
                input[specialIdx] * (2.0f / numPoints) * lambda_k * lambda_v * cos_i_k * cos_j_v;
          }
        }
      }
    }
...
template <typename scalar_t>
__global__ void cudaNaiveIDCT2DKernel(const uint numTotalThreads, const uint batchSizeDim,
                                      const uint channelDim, const uint heightDim,
                                      const uint widthDim, const scalar_t* __restrict__ input,
                                      const uint numPoints, scalar_t* __restrict__ output) {
  const uint idx =
      ...
      for (uint j = 0; j < numPoints; j++) {
        ...
        for (uint k = 0; k < numPoints; k++) {
          ...
          for (uint v = 0; v < numPoints; v++) {
            ...
            output[specialIdx] +=
                input[spectralIdx] * (2.0f / numPoints) * lambda_k * lambda_v * cos_i_k * cos_j_v;
          }
        }
      }
    }
