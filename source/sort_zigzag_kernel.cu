    ...
    if (idx < numTotalThreads) {
        ...
        for (uint k = 0; k < numPoints; k++) {
            ...
            for (uint v = 0; v < numPoints; v++) {
                ...
                uint outputChannelIndex = priority == SORT_FREQUENCY_FIRST ? outputSpectralIndex * channelDim + c : c * numPointsPow2 + outputSpectralIndex;
    ...
    if (idx < numTotalThreads) {
        ...
        for (uint k = 0; k < numPoints; k++) {
            ...
            for (uint v = 0; v < numPoints; v++) {
                ...
                uint inputChannelIndex = priority == SORT_FREQUENCY_FIRST ? inputSpectralIndex * channelDim + c : c * numPointsPow2 + inputSpectralIndex;
