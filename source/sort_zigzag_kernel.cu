...
__global__ void cudaSortCoefficientsByZigzagKernel(
    const uint numTotalThreads, const uint batchSizeDim, const uint channelDim,
    const uint heightDim, const uint widthDim, const scalar_t* __restrict__ input,
    const uint numPoints, const uint* zigzag, const uint priority, scalar_t* __restrict__ output) {
  const uint idx =
  ...
  if (idx < numTotalThreads) {
    ...
    for (uint k = 0; k < numPoints; k++) {
      ...
      for (uint v = 0; v < numPoints; v++) {
        ...
        uint outputChannelIndex = priority == SORT_BY_FREQUENCIES
                                      ? outputSpectralIndex * channelDim + c
                                      : c * numPointsPow2 + outputSpectralIndex;
        uint inputIndex =
            n * chwDim * numPointsPow2 + c * hwDim * numPointsPow2 + hk * widthDim * numPoints + wv;
        uint outputIndex =
            n * chwDim * numPointsPow2 + outputChannelIndex * hwDim + h * widthDim + w;
...
template <typename scalar_t>
__global__ void cudaRecoverCoefficientsByZigzagKernel(
    const uint numTotalThreads, const uint batchSizeDim, const uint channelDim,
    const uint heightDim, const uint widthDim, const scalar_t* __restrict__ input,
    const uint numPoints, const uint* zigzag, const uint priority, scalar_t* __restrict__ output) {
  const uint idx =
  ...
  if (idx < numTotalThreads) {
    ...
    for (uint k = 0; k < numPoints; k++) {
      ...
      for (uint v = 0; v < numPoints; v++) {
        ...
        uint inputChannelIndex = priority == SORT_BY_FREQUENCIES
                                     ? inputSpectralIndex * channelDim + c
                                     : c * numPointsPow2 + inputSpectralIndex;
        uint outputIndex =
            n * chwDim * numPointsPow2 + c * hwDim * numPointsPow2 + hk * widthDim * numPoints + wv;
...
at::Tensor cudaSortCoefficientsByZigzag(const at::Tensor input, const uint numPoints,
                                        const uint priority) {
  ...
  CHECK_CUDA_ERROR(
      cudaMemcpy(zigzagGPU, zigzag, numPoints * numPoints * sizeof(uint), cudaMemcpyHostToDevice));

  at::Tensor output =
      at::zeros({batchSize, channel * numPoints * numPoints, height / numPoints, width / numPoints},
                input.options());
  ...
  AT_DISPATCH_FLOATING_TYPES(input.type(), "cudaSortCoefficientsByZigzag", ([&] {
                               cudaSortCoefficientsByZigzagKernel<scalar_t>
                                   <<<numBlocks, threadsPerBlock>>>(
                                       ...
                                       zigzagGPU, priority, output.data_ptr<scalar_t>());
                             }));
...
at::Tensor cudaRecoverCoefficientsByZigzag(const at::Tensor input, const uint numPoints,
                                           const uint priority) {
  at::IntList inputSize = input.sizes();
  int batchSize = inputSize[0];
  int channel = inputSize[1];
  ...
  at::Tensor output = at::zeros(
      {batchSize, channel / (numPoints * numPoints), height * numPoints, width * numPoints},
      input.options());
  ...
  AT_DISPATCH_FLOATING_TYPES(
      input.type(), "cudaRecoverCoefficientsByZigzag", ([&] {
        cudaRecoverCoefficientsByZigzagKernel<scalar_t><<<numBlocks, threadsPerBlock>>>(
            numTotalThreads, batchSize, channel / (numPoints * numPoints), height, width,
            input.data_ptr<scalar_t>(), numPoints, zigzagGPU, priority,
