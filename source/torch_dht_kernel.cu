...
__global__ void cudaNaiveDHT2DKernel(const uint numTotalThreads, const uint batchSizeDim, const uint channelDim, const uint heightDim, const uint widthDim, const scalar_t* __restrict__ input, const uint numPoints, scalar_t* __restrict__ output) {
        ...
        for (uint k = 0; k < numPoints; k++) {
            ...
            for (uint v = 0; v < numPoints; v++) {
                ...
                for (uint i = 0; i < numPoints; i++) {
                    ...
                    float sin_cos_i_k = cosf(2.0f * M_PI * i * k / numPoints) 
                                      + sinf(2.0f * M_PI * i * k / numPoints);

                    for (uint j = 0; j < numPoints; j++) {
                        uint wj = w * numPoints + j;
                        float sin_cos_j_v = cosf(2.0f * M_PI * j * v / numPoints) 
                                          + sinf(2.0f * M_PI * j * v / numPoints); 
...
template <typename scalar_t>
__global__ void cudaNaiveIDHT2DKernel(const uint numTotalThreads, const uint batchSizeDim, const uint channelDim, const uint heightDim, const uint widthDim, const scalar_t* __restrict__ input, const uint numPoints, scalar_t* __restrict__ output) {
