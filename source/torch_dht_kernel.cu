...
at::Tensor cudaNaiveDHT2D(const at::Tensor input, const uint points, const bool sortbyZigzag) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.type(), "cudaNaiveDHT2D", ([&] {
                                        cudaNaiveDHT2DAndSortCoefficientsByZigzagKernel<scalar_t>
...
at::Tensor cudaNaiveIDHT2D(const at::Tensor input, const uint points, const bool recoverbyZigzag) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.type(), "cudaNaiveIDHT2D", ([&] {
        cudaNaiveIDHT2DAndRecoverCoefficientsByZigzagKernel<scalar_t>
            <<<numBlocks, threadsPerBlock, p2 * sizeof(uint)>>>(
