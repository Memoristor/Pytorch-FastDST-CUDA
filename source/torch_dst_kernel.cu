...
__global__ void cudaNaiveDST2DKernel(const uint numTotalThreads, const uint batchSizeDim,
...
template <typename scalar_t>
__global__ void cudaNaiveIDST2DKernel(const uint numTotalThreads, const uint batchSizeDim,
                                      const uint channelDim, const uint heightDim,
                                      const uint widthDim, const scalar_t* __restrict__ input,
                                      const uint numPoints, scalar_t* __restrict__ output) {
  const uint idx =
